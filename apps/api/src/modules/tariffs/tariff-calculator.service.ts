...
export class TariffCalculatorService {
  ...
  async findTariff(input: FindTariffDto) {
        ...
        rows: {
          where: {
            AND: [
              ...
              { OR: [{ fromLocationId: null }, { fromLocationId: input.fromLocationId }] },
              { OR: [{ toLocationId: null }, { toLocationId: input.toLocationId }] },
              { OR: [{ containerTypeId: null }, { containerTypeId: input.containerTypeId }] },
    ...
    return { tariffFound: true, tariffId: tariff.id, tariffRowId: row.id, price: row.price, unit: row.unit, currency: row.currency, tariffName: tariff.name };
  }
