...
export class CalculatorService {
  ...
  async quote(input: CreateQuoteDto) {
    ...
    const additionalServicesCost = await this.resolveAdditionalServices(input, direction === 'KLD_OUT' ? kmtp.id : pkt.id, container.id, date, warnings);
    ...
    const breakdown = [['Базовый тариф LI-LO', costs.liLo], ['ПРР в порту отправки', costs.portHandlingOrigin], ['FIOS', costs.fios], ['ПРР в порту прибытия', costs.portHandlingDestination], ...(input.category === 'REF' ? [['Подключения на терминале', costs.terminalConnections] as const] : []), ['Пользование контейнером', costs.containerUsage], ['Хранение контейнера', costs.containerStorage], ['Дополнительные услуги', costs.additionalServices], ['Маржа базового тарифа LI-LO', margin], ['Маржа базового тарифа LI-LO, %', costs.liLo ? margin / costs.liLo * 100 : 0], ['Прочие расходы', costs.otherExpenses], ['Первая миля', costs.firstMile], ['Последняя миля', costs.lastMile], ['Идентификация', costs.identification], ['Отсрочка платежа, дней', input.paymentDelayDays], ['Стоимость денег', costs.moneyCost], ['Итого', costs.total]].filter(([, value]) => value !== null).map(([label, value]) => ({ label: String(label), value: String(label).endsWith(', %') ? `${Number(value).toFixed(1)} %` : String(label) === 'Отсрочка платежа, дней' ? String(value) : formatRubles(Number(value)), emphasis: String(label).startsWith('Маржа') || String(label) === 'Итого' }));
  ...
  private async resolveAdditionalServices(input: CreateQuoteDto, originTerminalId: number, containerId: number, date: Date, warnings: string[]) {
    const special = originTerminalId === 1 && input.owner === 'SOC' && [1, 2].includes(containerId);
