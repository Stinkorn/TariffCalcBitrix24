...
export class CalculatorService {
  ...
  async quote(input: CreateQuoteDto) {
    ...
    const otherExpenses = (first.missing || last.missing) ? null : first.amount + last.amount + eaeuConfirmation + paymentDelayCost;
    ...
    const commercial = commercialQuote(saleRate, costs, input.category);
...
function commercialQuote(saleRate: number | null, costs: any, category: string) { const serviceBase = (costs.fios ?? 0) + (costs.firstMile ?? 0) + (costs.lastMile ?? 0) + (costs.containerUsage ?? 0); const services = 0.15 * serviceBase; if (saleRate === null || saleRate === 0 || costs.total === null) return { forwardingMargin: '—', forwardingMarginPercent: '—', servicesMargin: formatRubles(services), totalMargin: '—', totalMarginPercent: '—' }; const other = costs.otherExpenses ?? 0; const liLo = costs.liLo ?? 0; const margin = liLoMargin(costs, category); const forwarding = saleRate - other - liLo + margin; const total = forwarding - services; return { forwardingMargin: formatRubles(forwarding), forwardingMarginPercent: formatPercent(forwarding / saleRate), servicesMargin: formatRubles(services), totalMargin: formatRubles(total), totalMarginPercent: formatPercent(total / saleRate) }; }
