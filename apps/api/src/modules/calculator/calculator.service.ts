...
export class CalculatorService {
  ...
  async quote(input: CreateQuoteDto) {
    ...
    const selectedServicesCost = selectedServices.reduce((sum, item) => sum + (item.amountValue ?? 0), 0);
    additionalServices.push({ name: 'Дополнительные услуги', active: additionalServicesCost > 0, amountValue: additionalServicesCost, priced: true });
    const seaTotal = sea.stage.amountValue;
    const identification = input.services.identification ? await this.ruleValue('IDENTIFICATION', date, warnings) : 0;
    ...
    const paymentBase = first.amount + seaTotal + last.amount + additionalServicesCost + selectedServicesCost + identification;
    const baseDoorToDoor = paymentBase + paymentDelayCost;
    const costs = { firstMile: first.missing ? null : first.amount, liLo: sea.missing ? null : sea.liLo, portHandlingOrigin: sea.missing ? null : sea.portHandlingOrigin, fios: sea.missing ? null : sea.fios, portHandlingDestination: sea.missing ? null : sea.portHandlingDestination, connectionOrigin: input.category === 'REF' ? sea.connectionOrigin : 0, connectionDestination: input.category === 'REF' ? sea.connectionDestination : 0, terminalConnections, containerUsage: sea.missing ? null : sea.containerUsage, containerStorage: sea.missing ? null : sea.containerStorage, additionalServices: additionalServicesCost, lastMile: last.missing ? null : last.amount, identification, eaeuConfirmation: 0, moneyCost: paymentDelayCost, otherExpenses: (first.missing || last.missing) ? null : first.amount + last.amount + identification + selectedServicesCost + paymentDelayCost, paymentDelay: paymentDelayCost, total: (first.missing || sea.missing || last.missing) ? null : baseDoorToDoor };
    const saleRate = input.saleRate ?? null;
    const commercial = saleRate === null || saleRate === 0 || costs.total === null ? emptyCommercial() : commercialQuote(saleRate, costs, input.category);
    const margin = liLoMargin(costs, input.category);
    const breakdown = [['Базовый тариф LI-LO', costs.liLo], ['ПРР в порту отправки', costs.portHandlingOrigin], ...(input.category === 'REF' ? [['Подключение в порту отправки', costs.connectionOrigin], ['Кол-во дней бесплатного подключения', 0]] as const : []), ['FIOS', costs.fios], ['ПРР в порту прибытия', costs.portHandlingDestination], ...(input.category === 'REF' ? [['Подключение в порту прибытия', costs.connectionDestination], ['Кол-во дней бесплатного подключения', 0]] as const : []), ['Пользование контейнером', costs.containerUsage], ['Хранение контейнера', costs.containerStorage], ['Дополнительные услуги', costs.additionalServices], ['Маржа базового тарифа LI-LO', margin], ['Маржа базового тарифа LI-LO, %', costs.liLo ? margin / costs.liLo * 100 : 0], ['Прочие расходы', costs.otherExpenses], ['Первая миля', costs.firstMile], ['Последняя миля', costs.lastMile], ['Подтверждение статуса ЕАЭС (3135/6271)', costs.eaeuConfirmation], ['Отсрочка платежа, дней', input.paymentDelayDays], ['Стоимость денег', costs.moneyCost]].filter(([, value]) => value !== null).map(([label, value]) => ({ label: String(label), numericValue: Number(value), value: String(label).endsWith(', %') ? `${Number(value).toFixed(1)} %` : String(label) === 'Отсрочка платежа, дней' ? String(value) : formatRubles(Number(value)), emphasis: String(label).startsWith('Маржа') }));
...
function commercialQuote(saleRate: number, costs: any, category: string) { const other = costs.otherExpenses ?? 0; const liLo = costs.liLo ?? 0; const margin = liLoMargin(costs, category); const serviceBase = (costs.fios ?? 0) + (costs.firstMile ?? 0) + (costs.lastMile ?? 0) + (costs.containerUsage ?? 0); const forwarding = saleRate - other - liLo + margin; const services = 0.15 * serviceBase; const total = forwarding + services; return { forwardingMargin: formatRubles(forwarding), forwardingMarginPercent: formatPercent(forwarding / saleRate), servicesMargin: formatRubles(services), totalMargin: formatRubles(total), totalMarginPercent: formatPercent(total / saleRate) }; }
function liLoMargin(costs: any, category: string) { return (costs.liLo ?? 0) - (costs.portHandlingOrigin ?? 0) - (costs.fios ?? 0) - (costs.portHandlingDestination ?? 0) - (costs.containerUsage ?? 0) - (costs.containerStorage ?? 0) - (costs.additionalServices ?? 0) - (category === 'REF' ? (costs.terminalConnections ?? 0) : 0); }
